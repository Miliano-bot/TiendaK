    ...
    <div className="modal-overlay">
      <div className="modal">
        ...
        <div style={{ display:'flex',gap:8,marginBottom:12,flexWrap:'wrap' }}>
          {[1,5,10,20,50].map(v => {
            const redondeado = Math.ceil(total/v)*v
            return <button key={v} className="btn btn-ghost" style={{ flex:1,minWidth:48,fontSize:13 }} onClick={() => setPago(String(redondeado))}>${redondeado}</button>
          })}
        </div>
        {pago && (
...
export default function Ventas() {
  ...
  async function confirmarVenta(pagoCon, vuelto) {
    ...
    const { data: venta, error: e1 } = await supabase.from('ventas')
      .insert([{ idempresa:ID_EMPRESA, idcliente:clienteSel?.idcliente||null, total, fecha: new Date(new Date().getTime() - 5*60*60*1000).toISOString() }])
      .select().single()
    if (e1) { alert('Error al guardar venta: '+e1.message); setGuardando(false); return }
    ...
    if (e2) { alert('Error en detalle: '+e2.message); setGuardando(false); return }

    // 3. Actualizar stock y registrar SALIDA en inventariohistorico
  ...
  if (guardando) return (
    <div style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:300,gap:12 }}>
      ...
      <p style={{ color:'var(--text2)',fontSize:14 }}>Procesando venta...</p>
    </div>
  )
        ...
        <div>
          <div className="panel" style={{ marginBottom:12 }}>
            ...
            <div style={{ position:'relative' }}>
              <input className="search-input" placeholder="Buscar cliente..." value={busCliente} onChange={e=>{setBusCliente(e.target.value);if(clienteSel)setClienteSel(null)}} />
