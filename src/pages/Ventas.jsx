...
function ModalNuevoCliente({ onCreado, onClose }) {
  const [nombre,setNombre]=useState(''); const [tel,setTel]=useState(''); const [saving,setSaving]=useState(false)
  async function crear() {
    if (!nombre.trim()) return alert('El nombre es obligatorio')
    setSaving(true)
    const { data, error } = await supabase.from('clientes').insert([{ idempresa:ID_EMPRESA, nombre:nombre.trim(), telefono:tel }]).select().single()
  ...
  return (
    <div className="modal-overlay">
      <div className="modal">
        <h3>Nuevo cliente</h3>
        <div className="form-group"><label>Nombre *</label><input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Nombre completo" autoFocus /></div>
        <div className="form-group"><label>Teléfono</label><input value={tel} onChange={e=>setTel(e.target.value)} placeholder="+593..." /></div>
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-primary" onClick={crear} disabled={saving}>{saving?'Guardando...':'Crear'}</button>
        </div>
      </div>
    </div>
  )
...
function ModalCobro({ total, onConfirmar, onClose }) {
  ...
  const pagof = parseFloat(pago)||0
  const vuelto = pagof - total
  const ok = pagof >= total
  return (
    <div className="modal-overlay">
      <div className="modal">
        <h3>Cobrar venta</h3>
        <div style={{ background:'var(--bg3)',borderRadius:10,padding:'16px',marginBottom:16,textAlign:'center' }}>
        ...
        <div className="form-group">
          ...
          <input type="number" min={total} step="0.01" value={pago} onChange={e=>setPago(e.target.value)} placeholder={`Mínimo $${total.toFixed(2)}`} autoFocus style={{ fontSize:18,textAlign:'center' }} />
        </div>
        {/* Atajos rápidos */}
        <div style={{ display:'flex',gap:8,marginBottom:12,flexWrap:'wrap' }}>
          {[1,5,10,20,50].map(v => (
            <button key={v} className="btn btn-ghost" style={{ flex:1,minWidth:50,fontSize:13 }} onClick={() => setPago(String(Math.ceil(total/v)*v))}>${Math.ceil(total/v)*v}</button>
          ))}
        </div>
        {pago && (
          <div style={{ background:ok?'rgba(76,175,135,0.1)':'rgba(224,82,82,0.1)',border:`1px solid ${ok?'rgba(76,175,135,0.3)':'rgba(224,82,82,0.3)'}`,borderRadius:8,padding:'12px 16px',marginBottom:14 }}>
            {ok
              ? <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                  ...
                  <span style={{ fontSize:26,fontWeight:700,color:'var(--success)' }}>${vuelto.toFixed(2)}</span>
                </div>
        ...
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-primary" onClick={() => onConfirmar(pagof,vuelto)} disabled={!ok} style={{ flex:1 }}>✅ Confirmar venta</button>
        </div>
      </div>
    </div>
  )
...
export default function Ventas() {
  const [tab,           setTab]           = useState('productos') // productos | carrito
  ...
  async function fetchProductos() {
    const { data } = await supabase.from('productos')
      ...
      .eq('idempresa',ID_EMPRESA).eq('discontinuado',false).gt('cantidad',0)
    setProductos(data||[])
  }

  async function fetchClientes() {
    const { data } = await supabase.from('clientes').select('idcliente,nombre,telefono').eq('idempresa',ID_EMPRESA).order('nombre')
    setClientes(data||[])
  }
  ...
  useEffect(() => {
    ...
    setProdFiltrados(productos.filter(p => p.nombre.toLowerCase().includes(q) || (p.codigo_barras||'').includes(q)).slice(0,8))
  ...
  function agregar(prod) {
    setCarrito(prev => {
      ...
      if (existe) {
        if (existe.cantidad >= prod.cantidad_stock) { alert(`Solo hay ${prod.cantidad_stock} disponibles`); return prev }
  ...
  function cambiarCantidad(id, delta) {
    setCarrito(prev => {
      const nuevos = prev.map(i => {
        ...
        if (nueva <= 0) {
          if (confirm(`¿Quitar "${i.nombre}"?`)) return null
          return i
        }
  ...
  async function confirmarVenta(pagoCon, vuelto) {
    setModalCobro(false)
    const { data: venta, error: e1 } = await supabase.from('ventas')
      .insert([{ idempresa:ID_EMPRESA, idcliente:clienteSel?.idcliente||null, total }])
      .select().single()
    if (e1) { alert('Error: '+e1.message); return }

    const detalle = carrito.map(i => ({ idventa:venta.idventa, idproducto:i.idproducto, cantidad:i.cantidad, precio:i.precio }))
    const { error: e2 } = await supabase.from('ventasdetalle').insert(detalle)
    if (e2) { alert('Error detalle: '+e2.message); return }
  ...
  const clientesFiltrados = clientes.filter(c => c.nombre.toLowerCase().includes(busCliente.toLowerCase())).slice(0,5)

  if (ventaOk) return (
    <div style={{ maxWidth:380,margin:'40px auto',textAlign:'center',padding:'0 16px' }}>
      <div style={{ fontSize:70,marginBottom:12 }}>✅</div>
      <h2 style={{ fontSize:22,fontWeight:700,marginBottom:6 }}>¡Venta realizada!</h2>
      <p style={{ color:'var(--text2)',marginBottom:20 }}>{ventaOk.cliente}</p>
      ...
      {tab === 'productos' && (
        <div>
          ...
          <div style={{ display:'flex',gap:8,marginBottom:10 }}>
            <input ref={codigoRef} className="search-input" placeholder="Código de barras + Enter"
              value={codigoBus} onChange={e=>setCodigoBus(e.target.value)}
              onKeyDown={e => e.key==='Enter' && codigoBus.trim() && buscarCodigo(codigoBus.trim())} />
            <button className="btn btn-ghost" onClick={() => setEscaner(true)} style={{ fontSize:20,padding:'8px 12px' }}>📷</button>
          </div>

          {/* Buscador nombre */}
          <div style={{ position:'relative',marginBottom:14 }}>
            <input className="search-input" placeholder="🔍 Buscar por nombre..." value={busProducto} onChange={e=>setBusProducto(e.target.value)} />
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',gap:10 }}>
            {productos.map(p => (
              <div key={p.idproducto} onClick={() => agregar({...p,cantidad_stock:p.cantidad})}
                style={{ background:'var(--bg2)',borderRadius:10,padding:10,cursor:'pointer',border:'1px solid var(--border)',transition:'border-color 0.15s,transform 0.1s',active:{transform:'scale(0.97)'} }}
              ...
              >
                {p.imagen
                  ...
                  : <div style={{ width:'100%',height:70,background:'var(--bg3)',borderRadius:6,marginBottom:8,display:'flex',alignItems:'center',justifyContent:'center',fontSize:28 }}>📦</div>
                }
        ...
        <div>
          ...
          <div className="panel" style={{ marginBottom:12 }}>
            ...
            <div style={{ display:'flex',gap:8,marginBottom:8 }}>
              <button className={`btn ${!clienteSel?'btn-primary':'btn-ghost'}`} style={{ flex:1,fontSize:12 }} onClick={() => { setClienteSel(null); setBusCliente('') }}>
                Consumidor final
              </button>
              <button className="btn btn-ghost" style={{ fontSize:12 }} onClick={() => setModalCliente(true)}>+ Nuevo</button>
            </div>
            <div style={{ position:'relative' }}>
              <input className="search-input" placeholder="Buscar cliente..." value={busCliente}
                onChange={e => { setBusCliente(e.target.value); if(clienteSel) setClienteSel(null) }} />
              {busCliente && !clienteSel && clientesFiltrados.length > 0 && (
                <div className="dropdown">
                  {clientesFiltrados.map(c => (
                    <div key={c.idcliente} className="dropdown-item" onClick={() => { setClienteSel(c); setBusCliente(c.nombre) }}>
                      ...
                      {c.telefono && <span style={{ fontSize:11,color:'var(--text2)',marginLeft:'auto' }}>{c.telefono}</span>}
                    </div>
                  ))}
                </div>
              )}
          ...
          {/* Items */}
          {carrito.length === 0
            ? <div className="empty-state"><div className="empty-icon">🛒</div>Carrito vacío — agrega productos</div>
            : (
          ...
          <div className="panel" style={{ position:'sticky',bottom:0 }}>
            ...
            <div style={{ display:'flex',gap:8 }}>
              ...
              <button className="btn btn-primary" style={{ flex:2,padding:12,fontSize:15 }} onClick={() => setModalCobro(true)} disabled={carrito.length===0}>
