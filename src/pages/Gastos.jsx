...
import { formatFecha, hoyEC, inicioDiaEC, finDiaEC, getRangoFechas } from '../utils/fecha'
import { getPeriodo, estaPendiente } from '../hooks/useNotificaciones'

const ID_EMPRESA = 1
const CICLOS  = [{ id:'semanal',label:'Semanal',desc:'Cada semana' },{ id:'mensual',label:'Mensual',desc:'Cada mes' },{ id:'trimestral',label:'Trimestral',desc:'Cada 3 meses' },{ id:'anual',label:'Anual',desc:'Cada año' }]
const DIAS_MES = Array.from({length:28},(_,i)=>i+1)
const MESES    = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']
const EMPTY_U  = { idcategoria:'', descripcion:'', monto:'', fecha:hoyEC() }
const EMPTY_P  = { idcategoria:'', nombre:'', descripcion:'', monto_base:'', ciclo:'mensual', dia_pago:'1', mes_pago:'1', fecha_inicio:hoyEC(), fecha_fin:'' }
...
function GastosUnicos({ categorias, onCambio }) {
  ...
  useEffect(() => { fetchGastos() }, [periodo])

  async function fetchGastos() {
    setLoading(true)
    const { desde, hasta } = getRangoFechas(periodo)
    let q = supabase.from('gastos').select('*,categoriasgasto(nombre)').eq('idempresa',ID_EMPRESA).eq('tipo','unico').order('fecha',{ascending:false})
    if (desde) q = q.gte('fecha',desde).lte('fecha',hasta)
    const{data}=await q
    setGastos(data||[])
    setLoading(false)
  }

  function openNew()   { setForm(EMPTY_U); setEditId(null); setModal(true) }
  ...
  function close()     { setModal(false); setForm(EMPTY_U); setEditId(null) }
  ...
  const periodos=[{id:'hoy',label:'Hoy'},{id:'semana',label:'Semana'},{id:'mes',label:'Este mes'},{id:'anio',label:'Este año'},{id:'total',label:'Todo'}]
  const total = gastos.reduce((s,g)=>s+parseFloat(g.monto),0)

  return (
    <div>
      {/* Selector período */}
      <div style={{ display:'flex',gap:0,marginBottom:14,background:'var(--bg2)',borderRadius:10,padding:4,border:'1px solid var(--border)' }}>
        {periodos.map(p=>(
          <button key={p.id} onClick={()=>setPeriodo(p.id)} style={{ flex:1,padding:'7px 2px',borderRadius:8,border:'none',cursor:'pointer',fontSize:10,fontWeight:500,background:periodo===p.id?'var(--accent)':'transparent',color:periodo===p.id?'#fff':'var(--text2)',transition:'all 0.2s' }}>{p.label}</button>
        ))}
      </div>
...
function GastosProgramados({ categorias, onCambio }) {
  ...
  async function fetchTodo() {
    ...
    setProgramados(progs||[]); setPagos(pgs||[]); setLoading(false)
  }

  function openNew()   { setForm(EMPTY_P); setEditId(null); setModal(true) }
  function openEdit(g) { setForm({ idcategoria:g.idcategoria||'', nombre:g.nombre, descripcion:g.descripcion||'', monto_base:g.monto_base, ciclo:g.ciclo, dia_pago:g.dia_pago||'1', mes_pago:g.mes_pago||'1', fecha_inicio:g.fecha_inicio, fecha_fin:g.fecha_fin||'' }); setEditId(g.idprogramado); setModal(true) }
  function setF(f,v)   { setForm(p=>({...p,[f]:v})) }
  ...
  async function save() {
    ...
    if(error) alert('Error: '+error.message); else { setModal(false); setForm(EMPTY_P); setEditId(null); fetchTodo(); onCambio?.() }
    setSaving(false)
  }
  ...
  async function registrarPago(gasto, estado) {
    ...
    const ahora   = new Date().toLocaleString('sv-SE',{timeZone:'America/Guayaquil'}).replace(' ','T')+'-05:00'

    await supabase.from('pagos_gastos').upsert([{
      idprogramado: gasto.idprogramado, periodo,
      monto_pagado: estado==='pagado'?monto:null,
      estado, fecha_pago: estado==='pagado'?ahora:null,
      nota: notaPago.trim()||null,
    }],{onConflict:'idprogramado,periodo'})

    if (estado==='pagado') {
      await supabase.from('gastos').insert([{
        idempresa:ID_EMPRESA, idcategoria:gasto.idcategoria,
        descripcion:`${gasto.nombre} · ${periodo}`,
        monto, tipo:'unico', fecha:ahora,
      }])
    }
  ...
  function getPagoActual(g) { return pagos.find(p=>p.idprogramado===g.idprogramado&&p.periodo===getPeriodo(g.ciclo)) }
  ...
  return (
    <div>
      <div style={{ display:'flex',justifyContent:'flex-end',marginBottom:14 }}>
        <button className="btn btn-primary" onClick={openNew}>+ Nuevo recurrente</button>
      </div>

      {pendientes.length>0&&(
        <div style={{ marginBottom:16 }}>
          <p style={{ fontSize:13,fontWeight:600,color:'var(--warn)',marginBottom:10 }}>⏰ Pendientes de pago</p>
          {pendientes.map(g=>(
      ...
      {modal&&(
        <Modal title={editId?'Editar recurrente':'Nuevo gasto recurrente'} onClose={()=>{setModal(false);setForm(EMPTY_P);setEditId(null)}} onSave={save}>
          ...
          <div className="form-group"><label>Ciclo *</label>
      ...
      {modalPago&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setModalPago(null)}>
          <div className="modal">
            ...
            <div className="form-group"><label>Monto pagado ($)</label>
              <input type="number" min="0" step="0.01" value={montoPago} onChange={e=>setMontoPago(e.target.value)} placeholder={String(modalPago.monto_base)} /></div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={()=>registrarPago(modalPago,'pagado')} disabled={saving}>{saving?'Guardando...':'✅ Confirmar pago'}</button>
            </div>
          </div>
        </div>
      )}
...
export default function Gastos({ onCambio }) {
  const [tab,setCambio]=useState('unicos')
  const [categorias,setCategorias]=useState([])
  useEffect(()=>{ supabase.from('categoriasgasto').select('*').eq('activo',true).order('nombre').then(({data})=>setCategorias(data||[])) },[])
  return (
    <div>
      <div style={{ display:'flex',gap:0,marginBottom:20,background:'var(--bg2)',borderRadius:10,padding:4,border:'1px solid var(--border)' }}>
        {[['unicos','💳 Pagos únicos'],['programados','🔄 Recurrentes']].map(([id,label])=>(
          <button key={id} onClick={()=>setCambio(id)} style={{ flex:1,padding:'9px 8px',borderRadius:8,border:'none',cursor:'pointer',fontSize:13,fontWeight:500,background:tab===id?'var(--accent)':'transparent',color:tab===id?'#fff':'var(--text2)',transition:'all 0.2s' }}>{label}</button>
        ))}
      </div>
